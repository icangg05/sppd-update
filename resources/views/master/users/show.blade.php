  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="md:col-span-1 space-y-6">
      <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col items-center text-center">
        ...
        {{-- Avatar Ring --}}
        <div class="w-24 h-24 bg-cyan-100 text-cyan-700 rounded-full flex items-center justify-center font-black text-3xl mb-4 ring-4 ring-cyan-50 shadow-inner">
          {{ strtoupper(substr($user->name, 0, 1)) }}
        </div>

        <h3 class="text-base font-bold text-slate-900 mb-0.5 px-2">{{ $user->name }}</h3>
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">{{ $user->position?->name ?? 'Pegawai' }}</p>
        ...
        <div class="w-full pt-4 border-t border-slate-100 flex flex-col gap-3">
          ...
          <div class="flex justify-center">
            @if ($user->is_active)
              <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-emerald-700 ring-1 ring-inset ring-emerald-600/20">
                <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span> Aktif
              </span>
            @else
              <span class="inline-flex items-center gap-1.5 rounded-full bg-rose-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-rose-700 ring-1 ring-inset ring-rose-600/10">
          ...
          <div class="mt-1 flex flex-wrap justify-center gap-1">
            @foreach ($user->roles as $role)
              <span class="inline-flex items-center rounded-md bg-slate-50 px-2 py-1 text-[11px] font-bold text-slate-600 ring-1 ring-inset ring-slate-500/10">
    ...
    <div class="md:col-span-2">
      <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wide mb-5 pb-2 border-b border-slate-100 flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6">
          ...
          <div class="space-y-1">
            ...
            <p class="text-sm font-mono font-semibold text-slate-800 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 inline-block">{{ $user->nip ?? '-' }}</p>
          </div>

          <div class="space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nomor Induk Kependudukan (NIK)</label>
            <p class="text-sm font-mono font-semibold text-slate-800 {{ $user->nik ? 'bg-slate-50 border-slate-100' : 'bg-amber-50/50 border-amber-100 text-amber-700' }} px-2.5 py-1.5 rounded-lg border inline-block">{{ $user->nik ?? 'Belum Diatur' }}</p>
          </div>

          <div class="space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Username</label>
            <p class="text-sm font-medium text-slate-800"><i class="fa-solid fa-at mr-1.5 text-slate-400"></i>{{ $user->username }}</p>
          </div>

          <div class="space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Alamat Email</label>
            <p class="text-sm font-medium text-slate-800"><i class="fa-regular fa-envelope mr-1.5 text-slate-400"></i>{{ $user->email }}</p>
          </div>

          <div class="space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">No. Telepon / WhatsApp</label>
            <p class="text-sm font-medium text-slate-800"><i class="fa-solid fa-phone mr-1.5 text-slate-400"></i>{{ $user->phone ?? '-' }}</p>
          </div>
          ...
          <div class="space-y-1">
            ...
            <p class="text-sm font-semibold text-slate-800"><i class="fa-solid fa-user-tier mr-1.5 text-slate-400"></i>{{ $user->employee_type->label() }}</p>
          </div>

          <div class="space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Golongan / Pangkat</label>
            <p class="text-sm font-semibold text-slate-800">
              <i class="fa-solid fa-award mr-1.5 text-slate-400"></i>{{ $user->rank ? $user->rank->group . ' — ' . $user->rank->name : '-' }}
            </p>
          </div>

          <div class="sm:col-span-2 space-y-1">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Instansi / Unit Kerja (OPD)</label>
