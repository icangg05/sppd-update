      ...
      <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-6">
            ...
            <div class="space-y-1">
              <x-form.input type="email" name="email" label="Email Resmi" :value="old('email', $user->email)" required
                class="focus:border-cyan-500 focus:ring-cyan-500" />
            </div>
            ...
            <div class="space-y-1">
              <x-form.input name="nik" label="NIK (Nomor Induk Kependudukan)" :value="old('nik', $user->nik)"
                placeholder="16 digit angka" required class="font-mono focus:border-cyan-500 focus:ring-cyan-500" />
            </div>
            ...
            <div class="space-y-1">
              <x-form.select name="employee_type" label="Tipe Status Pegawai" required
                ...
                @foreach (\App\Enums\EmployeeType::cases() as $type)
                  <option value="{{ $type->value }}" {{ old('employee_type', $user->employee_type->value) === $type->value ? 'selected' : '' }}>
                    {{ $type->label() }}
                  </option>
                @endforeach
              </x-form.select>
            ...
            <div class="space-y-1">
              <x-form.select name="department_id" label="Instansi / Unit Kerja (OPD)"
                ...
                @foreach ($departments as $d)
                  <option value="{{ $d->id }}" {{ old('department_id', auth()->user()->hasRole('super_admin') ? $user->department_id : auth()->user()->department_id) == $d->id ? 'selected' : '' }}>
                    {{ $d->display_name }}
                  </option>
                @endforeach
              </x-form.select>
