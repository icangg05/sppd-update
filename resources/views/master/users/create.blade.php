    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <select name="employee_type" class="form-select" required>
          @foreach(\App\Enums\EmployeeType::cases() as $type)
            <option value="{{ $type->value }}" {{ old('employee_type') === $type->value ? 'selected' : '' }}>{{ $type->label() }}</option>
          @endforeach
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <select name="rank_id" class="form-select">
          <option value="">— Pilih —</option>
          @foreach($ranks as $r)
            <option value="{{ $r->id }}" {{ old('rank_id') == $r->id ? 'selected' : '' }}>{{ $r->group }} — {{ $r->name }}</option>
          @endforeach
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <input type="text" name="position_name" value="{{ old('position_name') }}" class="form-input" placeholder="Misal: Staf Umum">
      </div>
      <div>
