          ...
          <div class="md:col-span-2 space-y-1.5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 p-2.5 bg-slate-50 border border-slate-200 rounded text-xs">
              @foreach($domains as $domain)
                <x-form.checkbox name="destination[]" :value="$domain->value" :checked="is_array(old('destination')) && in_array($domain->value, old('destination'))" label="{{ $domain->label() }}" wrapper-class="flex items-center gap-2 font-medium text-slate-700 cursor-pointer" />
              @endforeach
            </div>
