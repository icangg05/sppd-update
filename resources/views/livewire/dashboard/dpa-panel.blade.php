<div class="flex flex-col gap-4">
  ...
  <div class="rounded border border-slate-200 bg-white shadow-md">
    <div class="flex items-center justify-between border-b border-slate-100 p-4">
      <h3 class="flex items-center gap-2 text-sm font-bold text-slate-800">
        <i class="fa-solid fa-list-ul text-cyan-500"></i> Rincian per Program / Kegiatan
      </h3>
    ...
    <div class="max-h-104 divide-y divide-slate-100 overflow-y-auto">
        ...
        <div class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="flex items-center gap-3 sm:w-64">
            ...
            <button type="button" wire:click="openDetail({{ $item['id'] }})" wire:loading.attr="disabled"
              ...
              class="inline-flex shrink-0 items-center gap-1.5 rounded border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-cyan-50 hover:text-cyan-700 disabled:opacity-50">
      ...
      @empty
        <div class="flex flex-col items-center justify-center p-8 text-slate-400">
          <i class="fa-solid fa-folder-open mb-3 text-3xl text-slate-200"></i>
  ...
  <x-ui.modal show="$wire.showDetail" maxWidth="max-w-2xl" icon="fa-solid fa-magnifying-glass-chart"
    ...
    @if ($detail)
      ...
      <div class="mb-4 grid grid-cols-3 gap-3">
        ...
        <div class="rounded border border-cyan-100 bg-cyan-50/60 p-2.5">
          <p class="text-[10px] font-semibold uppercase tracking-wide text-cyan-500">Realisasi</p>
          <p class="mt-0.5 font-mono text-sm font-bold text-cyan-700">Rp {{ number_format($detail['realisasi'], 0, ',', '.') }}</p>
        </div>
        <div class="rounded border p-2.5 {{ $detail['sisa'] < 0 ? 'border-rose-100 bg-rose-50' : 'border-emerald-100 bg-emerald-50/60' }}">
          <p class="text-[10px] font-semibold uppercase tracking-wide {{ $detail['sisa'] < 0 ? 'text-rose-500' : 'text-emerald-500' }}">Sisa</p>
      ...
      <div class="mb-2 flex items-baseline justify-between gap-2">
        ...
        @if ($detail['total'] > $detail['shown'])
          <p class="text-[11px] text-slate-400">Menampilkan {{ $detail['shown'] }} kontributor terbesar</p>
        @endif
      </div>
      <div class="max-h-72 divide-y divide-slate-100 overflow-y-auto rounded border border-slate-100">
        ...
        @empty
          <p class="p-6 text-center text-sm text-slate-400">Belum ada SPPD disetujui/selesai pada item ini.</p>
        @endforelse
      </div>
    @else
    ...
    <x-slot:footer class="flex justify-end">
      <button type="button" @click="$wire.showDetail = false"
        class="rounded border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
        Tutup
      </button>
    </x-slot:footer>
  </x-ui.modal>
