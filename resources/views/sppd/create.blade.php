        ...
        try {
          ...
          if (isComplete && data.steps.length > 0) {
            ...
            if (!data.has_header) {
              ...
              $('#error-text').text('Aturan alur verifikasi belum dikonfigurasi untuk kriteria pemohon atau tujuan ini.');
            } else {
              $('#error-text').text('Ada pejabat struktural yang belum ditentukan dalam alur ini. Harap lengkapi struktur organisasi di menu Unit Kerja.');
            }
          }
        } catch (error) {
          $stepsContainer.html('<div class="col-span-full text-center py-4 text-sm font-medium text-red-600"><i class="fa-solid fa-circle-exclamation mr-1"></i> Gagal memuat data validasi alur.</div>');
        }
      }
