...
<div class="page-header">
  ...
  <div class="flex gap-2">
    @if($sppd->status->value === 'in_progress' && (auth()->id() === $sppd->creator_id || auth()->id() === $sppd->user_id))
      <form action="{{ route('sppd.destroy', $sppd) }}" method="POST" onsubmit="return confirm('Batalkan dan hapus pengajuan SPPD ini?')">
        @csrf
        @method('DELETE')
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
  ...
  <div class="xl:col-span-2 space-y-6">
    ...
    <div class="card p-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
        ...
        <div>
          ...
          <p class="font-medium text-slate-900">{{ $sppd->start_date->format('d M Y') }} — {{ $sppd->end_date->format('d M Y') }}</p>
    ...
    <div class="card p-6">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
          ...
          <div>
            <p class="font-medium text-slate-900">{{ $dest->province->name }}{{ $dest->regency ? ', ' . $dest->regency->name : '' }}</p>
            @if($dest->address)<p class="text-xs text-slate-500">{{ $dest->address }}</p>@endif
          </div>
        </div>
        @endforeach
      </div>
    ...
    <div class="card p-6">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <span class="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg text-sm">
          <span class="w-6 h-6 bg-primary-500 rounded-full flex items-center justify-center text-[10px] font-bold text-white">{{ strtoupper(substr($f->user->name,0,1)) }}</span>
          {{ $f->user->name }}
        </span>
        @endforeach
      </div>
    ...
    <div class="card overflow-hidden">
      ...
      <table class="data-table">
        <thead><tr><th>Uraian</th><th class="text-right">Biaya Satuan</th><th class="text-right">Qty</th><th class="text-right">Subtotal</th></tr></thead>
        <tbody>
          @php $total = 0; @endphp
          @foreach($sppd->costDetails as $c)
            @php $sub = $c->unit_cost * $c->quantity; $total += $sub; @endphp
            <tr>
              <td>{{ $c->description }}</td>
  ...
  <div class="space-y-6">
      ...
      @if($sppd->approvals->count())
        <div>
          ...
          <div class="timeline-step">
            <div class="timeline-dot {{ $ap->status->value === 'approved' ? 'bg-emerald-500' : ($ap->status->value === 'rejected' ? 'bg-red-500' : ($ap->status->value === 'revision' ? 'bg-orange-500' : 'bg-slate-300')) }}">
            ...
            <div class="flex-1 min-w-0">
              <p class="font-medium text-sm text-slate-900">{{ $ap->role_label }}</p>
              <p class="text-xs text-slate-500">{{ $ap->approver->name }}</p>
              <span class="badge-{{ $ap->status->value }} mt-1">{{ $ap->status->label() }}</span>
              @if($ap->notes)<p class="text-xs text-slate-500 mt-1 italic">"{{ $ap->notes }}"</p>@endif
              @if($ap->acted_at)<p class="text-[11px] text-slate-400 mt-1">{{ $ap->acted_at->format('d M Y H:i') }}</p>@endif
            </div>
          </div>
          @endforeach
        </div>
    ...
    @php
      $myApproval = $sppd->approvals->where('approver_id', auth()->id())->where('status', \App\Enums\ApprovalStatus::PENDING)->first();
    @endphp
    @if($myApproval)
    <div class="card p-6 border-amber-200 bg-amber-50">
      <h3 class="font-semibold text-amber-900 mb-3">Menunggu Keputusan Anda</h3>
      <p class="text-sm text-amber-700 mb-4">Sebagai <strong>{{ $myApproval->role_label }}</strong> (Step {{ $myApproval->step_order }})</p>
