...
<div class="flex flex-col gap-4 p-1">
  ...
  <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
    ...
    <x-ui.button href="{{ route('sppd.create') }}" variant="secondary" class="inline-flex items-center gap-2 rounded border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
      <x-slot name="icon">
  ...
  <form action="{{ route('sppd.store') }}" method="POST" enctype="multipart/form-data" id="form-sppd-store">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <div class="rounded border border-cyan-200 bg-cyan-50/50 p-4 shadow-sm leading-tight flex flex-col justify-between">
        ...
        <div class="mt-4 pt-3 border-t border-cyan-200/60">
          ...
          <span class="inline-block mt-1.5 rounded-sm bg-white border border-cyan-200 px-2 py-0.5 text-xs font-bold uppercase tracking-wide text-cyan-800">
        ...
        <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
          @foreach ($steps as $step)
            <div class="flex items-center gap-2.5 rounded border border-emerald-200 bg-emerald-50/50 p-2.5 shadow-2xs">
              <div class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-xs font-bold text-white shadow-2xs">
                {{ $loop->iteration }}
              </div>
              <div class="min-w-0 leading-tight">
                <p class="text-xs font-bold uppercase tracking-wide text-emerald-600">{{ $step['role_label'] }}</p>
                <p class="text-sm font-semibold text-emerald-950 truncate mt-0.5" title="{{ $step['approver_name'] }}">{{ $step['approver_name'] }}</p>
              </div>
            </div>
          @endforeach
        </div>
      ...
      <div class="p-5 space-y-5">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <x-form.input
            ...
            required
          />
          <x-form.input type="date" name="start_date" label="Tanggal Berangkat" value="{{ old('start_date', date('Y-m-d')) }}" required />
          <x-form.input type="date" name="end_date" label="Tanggal Kembali" value="{{ old('end_date', date('Y-m-d')) }}" required />
        </div>
        ...
        <div>
          ...
          @if ($domain === 'dalam_daerah')
            <div id="dalam-daerah-fields" class="flex flex-col gap-2">
              <div class="inline-flex items-center gap-1.5 rounded-sm bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 border border-slate-200/60 w-fit">
                <i class="fa-solid fa-location-dot text-slate-400"></i>
                Lokasi Basis: <span class="font-bold text-slate-700">Kota Kendari, Sulawesi Tenggara</span>
              </div>
              <input type="text" name="destinations[0][address_only]" class="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 placeholder-slate-400 shadow-2xs focus:border-cyan-500 focus:outline-hidden focus:ring-1 focus:ring-cyan-500" required placeholder="Sebutkan instansi/tempat tujuan spesifik (misal: Kantor Gubernur, Kecamatan Poasia)">
            </div>
          @else
            <div id="multi-dest-fields">
              <div id="dest-wrap" class="space-y-2.5">
                <div class="dest-row grid grid-cols-1 {{ $domain === 'lddp' ? 'md:grid-cols-2' : 'md:grid-cols-3' }} gap-2 p-3 bg-slate-50/80 border border-slate-200 rounded">
                  <div class="{{ $domain === 'lddp' ? 'hidden' : '' }}">
                    <select name="destinations[0][province_id]" class="w-full rounded border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-700 prov-sel" required>
                  ...
                  <div>
                    <select name="destinations[0][regency_id]" class="w-full rounded border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-700 reg-sel" required>
                      <option value="">— Kabupaten/Kota —</option>
                    </select>
                  </div>
                  <div>
                    <input type="text" name="destinations[0][address]" class="w-full rounded border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-800 placeholder-slate-400" placeholder="Instansi / Alamat Spesifik Tujuan" required>
                  </div>
                </div>
              </div>
              <div class="flex items-center gap-3 mt-2.5">
                <button type="button" id="btn-add-dest" class="text-xs text-cyan-600 font-bold hover:text-cyan-700 flex items-center gap-1">
                ...
                <span id="dest-max-info" class="hidden text-xs text-amber-600 font-semibold"><i class="fa-solid fa-circle-info"></i> Batas maksimal 4 tujuan</span>
              </div>
            </div>
          @endif
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 mt-4">
      ...
      <div class="rounded border border-slate-200 bg-white shadow-md overflow-hidden flex flex-col">
        <div class="border-b border-slate-100 bg-slate-50/75 px-5 py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h3 class="text-xs font-bold tracking-wider text-slate-600 uppercase flex items-center gap-1.5">
          ...
          <div class="relative w-full sm:w-48">
            ...
            <input type="text" id="search-follower" class="w-full rounded border border-slate-300 bg-white py-1 pl-8 pr-2.5 text-xs text-slate-700 placeholder-slate-400 focus:border-cyan-500 focus:outline-hidden" placeholder="Cari nama pegawai...">
          </div>
        </div>
          ...
          <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
            @foreach ($users as $u)
              @if ($u->id != $pelaksana->id)
                ...
                <label class="follower-item flex items-start gap-2.5 rounded border border-slate-200 bg-white p-2.5 shadow-2xs transition-colors hover:bg-slate-50 cursor-pointer {{ $isActive ? 'opacity-60 cursor-not-allowed bg-rose-50/50 border-rose-200' : '' }}">
                  ...
                  <div class="min-w-0 leading-tight">
                    ...
                    @if ($isActive)
                      <span class="inline-block text-[10px] text-rose-600 font-semibold mt-0.5"><i class="fa-solid fa-circle-exclamation"></i> Dinas Aktif</span>
                    @endif
                  </div>
                </label>
              @endif
    ...
    <div class="mt-4 rounded border border-slate-200 bg-white shadow-md overflow-hidden">
      ...
      <div class="p-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <x-form.input type="date" name="spt_date" label="Tanggal Penerbitan SPT" value="{{ old('spt_date', '2026-04-21') }}" />
        <x-form.input type="date" name="sppd_date" label="Tanggal Penerbitan SPPD" value="{{ old('sppd_date', '2026-04-23') }}" />
      </div>
    </div>

    {{-- Tombol Submit Pembuat Aksi --}}
    <div class="mt-5 flex justify-end">
      <button type="button" id="btn-trigger-confirm" class="inline-flex items-center gap-2 rounded bg-cyan-600 px-6 py-2.5 text-sm font-bold text-white shadow-md transition hover:bg-cyan-700">
...
<div id="modal-confirm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-2xs hidden transition-opacity duration-200 opacity-0">
  <div class="w-full max-w-md transform rounded border border-slate-200 bg-white p-5 shadow-xl transition-all leading-tight">
    ...
    <div class="py-4 space-y-3.5 text-sm text-slate-600">
      <div>
        ...
        <p id="confirm-pelaksana" class="font-bold text-slate-800 mt-1 bg-slate-50 px-2.5 py-1.5 rounded border border-slate-200/60"></p>
      </div>
      <div>
        <span class="block text-xs font-bold uppercase tracking-wider text-slate-400">Daftar Pengikut Dinas:</span>
        <div id="confirm-pengikut-container" class="mt-1 max-h-28 overflow-y-auto border border-slate-200 rounded divide-y divide-slate-100 bg-slate-50/50">
          {{-- Diisi via JS --}}
        </div>
      </div>
    ...
    <div class="flex justify-end gap-2 border-t border-slate-100 pt-3">
      <button type="button" id="btn-modal-close" class="rounded border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
        Periksa Kembali
      </button>
      <button type="button" id="btn-modal-submit" class="rounded bg-cyan-600 px-4 py-2 text-xs font-bold text-white hover:bg-cyan-700">
        Ya, Ajukan Sekarang
      </button>
    </div>
  </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function loadRegencies($provElement) {
        ...
        $.getJSON(`/api/provinces/${$provElement.val()}/regencies`, function(data) {
          ...
          $.each(data, function(i, item) {
            ...
            o += `<option value="${item.id}" ${item.id == currentVal ? 'selected' : ''}>${item.name}</option>`;
          });
          $r.html(o);
        });
      }
      ...
      $('#btn-trigger-confirm').on('click', function() {
        ...
        $('#confirm-pelaksana').html(`<i class="fa-solid fa-user text-cyan-600 mr-1.5"></i> ${$('#hidden_user_name').val()}`);
        ...
        $('.follower-cb:checked').each(function() {
          ...
          $listContainer.append(`<div class="px-3 py-2 text-xs font-semibold text-slate-700 bg-white"><i class="fa-solid fa-caret-right text-cyan-600 mr-1.5"></i> ${name}</div>`);
          checkedPengikut++;
        });

        if(checkedPengikut === 0) {
          $listContainer.html('<div class="px-3 py-2.5 text-xs text-slate-400 italic bg-white"><i class="fa-solid fa-user-minus mr-1"></i> Tidak ada pengikut</div>');
        }

        // Tampilkan Modal dengan animasi halus
        $modal.removeClass('hidden');
        setTimeout(() => { $modal.removeClass('opacity-0').addClass('opacity-100'); }, 20);
      });
