		...
		<div class="mb-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
			<div class="leading-tight">
				...
				<p class="mt-1 font-mono text-sm text-slate-500">
					<i class="fa-solid fa-hashtag mr-1 text-xs text-slate-400"></i>
		...
		<div class="space-y-6">
			...
			<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
				...
				<div class="grid grid-cols-1 gap-x-8 gap-y-5 p-5 sm:grid-cols-2">
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Pelaksana</p>
						<p class="text-sm font-bold text-slate-800">{{ $sppd->user->name }}</p>
						<p class="font-mono text-xs text-slate-500">{{ $sppd->user->nip ?? '-' }}</p>
					</div>
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Instansi Pengusul</p>
						<p class="text-sm font-semibold text-slate-800">{{ $sppd->budget?->department?->name ?? '-' }}</p>
					</div>
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Kategori Perjalanan</p>
						<p class="text-sm font-semibold text-slate-800">{{ $sppd->category?->name ?? '-' }}</p>
					</div>
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Domain Wilayah</p>
						<p class="text-sm font-semibold text-slate-800">
							<span class="rounded border border-cyan-100 bg-cyan-50 px-2 py-0.5 text-xs uppercase text-cyan-700">{{ $sppd->domain->label() }}</span>
						</p>
					</div>
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Sifat Surat Dokumen</p>
						<p class="text-sm font-semibold text-slate-800">
					...
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Tanggal Pelaksanaan</p>
						<p class="text-sm font-semibold text-slate-800">
							{{ $sppd->start_date->translatedFormat('d M Y') }}
							<i class="fa-solid fa-arrow-right mx-1 text-[10px] text-slate-400"></i>
					...
					<div>
						<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Pembuat Dokumen</p>
						<p class="text-sm font-semibold text-slate-800">{{ $sppd->creator?->name ?? '-' }}</p>
					</div>
					<div class="border-t border-slate-100 pt-3 sm:col-span-2">
						<p class="mb-1 text-xs font-bold uppercase tracking-wider text-slate-400">Maksud Perjalanan</p>
						<p class="text-sm font-medium leading-relaxed text-slate-800">{{ $sppd->purpose }}</p>
					</div>
					@if ($sppd->problem)
						<div class="border-t border-slate-100 pt-3 sm:col-span-2">
							<p class="mb-1 text-xs font-bold uppercase tracking-wider text-slate-400">Persoalan</p>
							<p class="rounded border border-slate-200 bg-slate-50 p-3 text-sm leading-relaxed text-slate-600">{{ $sppd->problem }}</p>
						</div>
					@endif
					@if ($sppd->facts)
						<div class="border-t border-slate-100 pt-3 sm:col-span-2">
							<p class="mb-1 text-xs font-bold uppercase tracking-wider text-slate-400">Fakta-Fakta Yang Mempengaruhi</p>
							<p class="rounded border border-slate-200 bg-slate-50 p-3 text-sm leading-relaxed text-slate-600">{{ $sppd->facts }}</p>
						</div>
					@endif
					@if ($sppd->analysis)
						<div class="border-t border-slate-100 pt-3 sm:col-span-2">
							<p class="mb-1 text-xs font-bold uppercase tracking-wider text-slate-400">Analisis</p>
							<p class="rounded border border-slate-200 bg-slate-50 p-3 text-sm leading-relaxed text-slate-600">{{ $sppd->analysis }}</p>
						</div>
					@endif
					@if ($sppd->notes)
						<div class="border-t border-slate-100 pt-3 sm:col-span-2">
							<p class="mb-1 text-xs font-bold uppercase tracking-wider text-slate-400">Catatan Tambahan</p>
			...
			@if ($sppd->budget)
				<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
					...
					<div class="grid grid-cols-1 gap-x-8 gap-y-5 p-5 sm:grid-cols-2">
						<div>
							<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">No. Rekening Anggaran</p>
							<p class="font-mono text-sm font-semibold text-slate-800">{{ $sppd->budget->account_code ?? '-' }}</p>
						</div>
						<div>
							<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Program</p>
							<p class="text-sm font-semibold text-slate-800">{{ $sppd->budget->program ?? '-' }}</p>
						</div>
						<div class="sm:col-span-2">
							<p class="mb-0.5 text-xs font-bold uppercase tracking-wider text-slate-400">Kegiatan</p>
					...
					<div class="flex flex-wrap gap-2.5 p-5">
						@foreach ($sppd->followers as $f)
							<div class="inline-flex items-center gap-2 rounded border border-slate-200 bg-white px-2 py-1.5">
								<span class="flex size-6 shrink-0 items-center justify-center rounded bg-cyan-600 text-[10px] font-bold text-white">
